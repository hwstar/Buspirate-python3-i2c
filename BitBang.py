...
class BBIO:
    ...
    def BBmode(self):
        if sys.platform == 'win32':
            ...
            self.resetBP()
            self.port.write(b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00")
            self.timeout(0.1)
    ...
    def setup_PWM(self, prescaler, dutycycle, period):
        ...
        self.port.write(bytes([prescaler]))
        self.port.write(bytes([(dutycycle >> 8) & 0xFF]))
        self.port.write(bytes([dutycycle & 0xFF]))
        self.port.write(bytes([(period >> 8) & 0xFF]))
        self.port.write(bytes([period & 0xFF]))
        self.timeout(0.1)
    ...
    def bulk_trans(self, byte_count=1, byte_string=None):
        ...
        self.port.write(bytes([0x10 | (byte_count - 1)]))  # self.timeout(0.1)
